...
class Road:
    def __init__(self, world):
        self.num_ctrl_points = (int)((world.win_height+SAFE_SPACE)/SPACING)+2 #число контрольных точек

        self.last_ctrl_point = 0 #последняя контрольная точка
        self.ctrl_points = [] #все конторольные точки
        self.centerPoints = [] #центральные точки
        self.pointsLeft = [] #точки слева
        self.pointsRight = [] #точки справа

        #заполнить контрльные точки
        for i in range(self.num_ctrl_points):
             self.ctrl_points.append(vect2d())

        #заполнить точки слева, справа и по центру
        ...
        #задать первые точки, чтобы дорога была прямая
        ...
        #для остальных точек задать случайные сегменты
    ...
    def calcBorders(self, i):
        prev_index = getPoint(i-1, self.num_ctrl_points*NUM_POINTS)#индекс предыдущего сегмента
        center = self.centerPoints[i]#текущая точка
        prev = self.centerPoints[prev_index]#предыдущая точка
        angle = atan2(center.x-prev.x, prev.y-center.y)#угол направления

        #отступы
        x = ROAD_WIDTH/2 * cos(angle)
        y = ROAD_WIDTH/2 * sin(angle)
        #добавить отступы в нужные стороны
    ...
    def createSegment(self, index):
        ...
        #задать p2
        seed()
        #координаты со случайным отступоа по х
        p2.co(p1.x + (random()-0.5)*MAX_DEVIATION, p1.y-SPACING)
        #случайны угол поворота
        p2.angle = MAX_ANGLE*(random()-0.5)

        #задать у координаты точек сегмента
        y_tmp = []
        for i in range(NUM_POINTS):
            y_tmp.append(p2.y+SPACING/NUM_POINTS*i)

        #получаем кубический сплайн центральной линии дороги
        ny = np.array([p2.y, p1.y]) #у координаты контрольных точек
        nx = np.array([p2.x, p1.x]) #х координаты контрольных точек
        cs = interpolate.CubicSpline(ny, nx, axis=0, bc_type=((1,p2.angle),(1,p1.angle))) #кубический сплайн с граничным условием для углов
        res = cs(y_tmp) #результат (х коорднаты)

        #заполнить центральные точки сегмента
        for i in range(NUM_POINTS):
            ...
            #заполнить границы для центральной точки
    ...
    #обновить сегменты при движении вверх
    ...
    def draw(self, world):
        ...
        if(ROAD_DBG):
            ...
            #рисовать границы
            for i in range(len(self.pointsLeft)):
                next_index = getPoint(i+1, NUM_POINTS*self.num_ctrl_points)#следующая точка

                p = self.pointsLeft[i]#текущая левая точка
                f = self.pointsLeft[next_index]#следующая левая точка
                #соединить точки (рисовать только наверх)
                if p.y >= f.y:
                    py.draw.line(world.win, BLACK, world.getScreenCoords(p.x, p.y), world.getScreenCoords(f.x, f.y), 4)

                #аналогично для правых точек
